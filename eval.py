...
def eval(val_txt, result_txt):
    ...
    fp = fp.cumsum()
    tp = tp.cumsum()
    rec = tp / npos     #tp为识别出来多少正确的框，npos为gt中共有多少框
    prec = tp / (fp+tp)  #fp为识别出来错误的框，fp+tp为识别出来多少框

    ap=0
    for t in range(11):
        thresh = 0.1*t
        value = prec[rec >= thresh]   #每次使用在所有阈值的Prec中，最大值的那个Precision值与Recall的变化值相乘
        if len(value) > 0:
            p = max(value)
            ap=ap+p/11
